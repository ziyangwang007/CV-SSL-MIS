...
parser.add_argument('--exp', type=str,
                    default='ACDC/Interpolation_Consistency_Training_Deep_Adversial', help='experiment_name')
...
def train(args, snapshot_path):
    ...
    optimizer_D = optim.Adam(DAN.parameters(), lr=args.DAN_lr, betas=(0.9, 0.99))
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            unlabeled_volume_batch_0 = unlabeled_volume_batch[:args.labeled_bs//2, ...]
            ...
            input_volume_batch = torch.cat(
                [labeled_volume_batch, batch_ux_mixed], dim=0) # 12 + 6 = 18
            ...
            supervised_loss = 2*loss_dice + loss_ce

            consistency_weight = get_current_consistency_weight(iter_num//150)
            
            DAN_outputs = DAN(outputs_soft[args.labeled_bs//2:], volume_batch[args.labeled_bs:])
            consistency_loss_ict = torch.mean((outputs_soft[args.labeled_bs:] - batch_pred_mixed) ** 2)
            consistency_loss_dan = F.cross_entropy(DAN_outputs, (DAN_target[:args.labeled_bs]).long())

            consistency_loss = consistency_loss_ict + 0.5*consistency_loss_dan
            loss = 1.5*supervised_loss + consistency_weight * consistency_loss
