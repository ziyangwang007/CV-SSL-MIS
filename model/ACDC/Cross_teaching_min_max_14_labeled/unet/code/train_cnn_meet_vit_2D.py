...
parser.add_argument('--exp', type=str,
                    default='ACDC/CNN_Meet_With_ViT', help='experiment_name')
...
def train(args, snapshot_path):
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            # mean teacher losses
            consistency_weight_cps = args.consistency * ramps.linear_rampup(iter_num // 150, args.consistency_rampup)
            consistency_weight_mt = args.consistency * ramps.linear_rampup(iter_num // 150, args.consistency_rampup)

            if iter_num < 1000:
                consistency_loss2 = 0.0
                consistency_loss1 = 0.0
            else:
                consistency_loss1 = torch.mean((outputs_soft1[args.labeled_bs:] - ema_output_soft) ** 2)
                consistency_loss2 = torch.mean((outputs_soft2[args.labeled_bs:] - ema_output_soft) ** 2)


            # adjust ratio of loss here
            model1_loss = loss1 + 7 * consistency_weight_cps * pseudo_supervision1 + consistency_weight_mt * consistency_loss1
            model2_loss = loss2 + 7 * consistency_weight_cps * pseudo_supervision2 + consistency_weight_mt * consistency_loss2
